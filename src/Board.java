...
public class Board {
    ...
    /**
     * Creates a Fillable board of size 7x6, 42 bits with underlying BitBoards of size 7x7, 49-bits for
    ...
    /**
     * Initializes map that stores each column as a key and the amount of empty spots (including
     * sentinel node row) per column (7 slots per column).
     */
    ...
    /**
     * Calls getNearestColIndex when the player's turn and places the piece in that column. The tree is
     * then updated to reflect the current board state. When it is the bot's turn it gets the next move
     * and the next move's column.
     * 
     * @param x
     * @param y
     * @return
    ...
    private int getSpecificCol(double x, double y) {
        ...
        if (turnCount % 2 == 0) {
            ...
            column = nextMove.getMask().getColUsingBitBoard(mask.getBit());
        }
    ...
    /**
     * Places the player's piece if the game is not finished and is within bounds and puts it into the
     * columnMap.
     * 
     * @param x
     * @param y
    ...
    /**
     * Updates the game tree based on whether a column is not full, and adds yellow's bit to a BitBoard
     * to determine whether a future game state wins the game. Determines whether the game is finished
     * or not.
     * 
     * @param column
     * @param row
    ...
    private void updateGameState(int column, int row) {
        if (!isColumnFull(column)) {
            ...
            BitBoard updatedMask = mask.addBitToMask(column);
            if (turnCount % 2 == 1) {
                yellow.setBit(yellow.addBitToPos(mask.getBit(), updatedMask.getBit()));
                if (yellow.checkWin()) {
                    gameOver(true);
                }
            } else {
    ...
    public void initializePieces(CanvasWindow canvas) {
        ...
        for (Fillable[] col : gameBoard) {
            ...
            for (Fillable var : col) {
                Fillable square = new Rectangle(xBoxMargin + (70 * (colCount % COLUMNS)),
                    yBoxMargin + (70 * (rowCount)), squareHeightAndWidth, squareHeightAndWidth);
    ...
    /**
     * Determines which player has what token. For the purposes of this project, the human player is red
     * and the bot is yellow.
     * 
     * @return
     */
